...
//   	}
	...
	public class GRITS
	{
		...
		public int PrintReport(string ReportID, object AdoCon, int AttributeCount, string[] Attributes, string[] Values)
		{
			...
			try
			{
				...
				try
				{
					...
					conn.ConnectionString="Server='"+AttributeDictionary["SERVER"].ToString()+"';Database='"+AttributeDictionary["DATABASE"].ToString()+"';Integrated Security=SSPI;Persist Security Info = false;Pooling=false";
					conn.Open();
					conn2 = new SqlConnection();
					conn2.ConnectionString="Server='"+AttributeDictionary["SERVER"].ToString()+"';Database='"+AttributeDictionary["DATABASE"].ToString()+"';Integrated Security=SSPI;Persist Security Info = false;Pooling=false";
					conn2.Open();
					try
					{
				...
				try
				{
					if(AttributeDictionary.ContainsKey("STARTDATE")&&null!=AttributeDictionary["STARTDATE"]&&AttributeDictionary.ContainsKey("ENDDATE")&&null!=AttributeDictionary["ENDDATE"])
					{
						...
						com.CommandText = 
							...
							"WHERE "+
							"p_Patient_Encounter.encounter_date >= '" + AttributeDictionary["STARTDATE"] + "' AND "+
							"p_Patient_Encounter.encounter_date < '" + System.DateTime.Parse(AttributeDictionary["ENDDATE"],System.Globalization.DateTimeFormatInfo.CurrentInfo).AddDays(1).ToShortDateString() + "' AND "+
							"p_Treatment_Item.treatment_type = 'IMMUNIZATION'";
					...
					foreach(GRITSData.CLIENTRow ClientRow in dsGRITSData.CLIENT.Rows)
					{
						...
						try
						{
							com.CommandText="SELECT cpr_id, first_name, last_name, date_of_birth, sex, "+
								"ssn, phone_number, address_line_1, address_line_2, city, state, zip FROM " +
								"p_Patient WHERE cpr_id='"+ClientRow.CPR_ID+"'";
							p_PatientReader = com.ExecuteReader(System.Data.CommandBehavior.SingleRow);

							if(!Historical)
							{
								com2.CommandText="SELECT TOP 1 result FROM p_observation_result poe WITH (NOLOCK) " +
									"inner join c_Observation_Tree WITH (NOLOCK) ON c_Observation_Tree.child_observation_id = poe.observation_id Where " +
									"result is not null and poe.cpr_id = '" + ClientRow.CPR_ID + "' and " +
									"c_Observation_Tree.parent_observation_id = '" + AttributeDictionary["PROGKEY_ELIG"] + 
							...
							if(!Historical&&!sdr_VaccineEligibility.Read())
							{
								...
								// skip patient
								//dsGRITSData.CLIENT.RemoveCLIENTRow(ClientRow);
								//continue; <--- Per chris, this is the beginning of the fix
							}
							...
							if(!DBNull.Value.Equals(p_PatientReader["ssn"])&&null!=p_PatientReader["ssn"])
							{
								ClientRow.SSN=p_PatientReader["ssn"].ToString().Replace("-","").Replace(" ","");
								if(ClientRow.SSN.Length!=9)
								{
									ClientRow.SSN=null;
								}
							}
							...
							if(!DBNull.Value.Equals(p_PatientReader["address_line_1"])&&null!=p_PatientReader["address_line_1"])
							{
								ClientRow.ADDRESS=p_PatientReader["address_line_1"].ToString();
								if(ClientRow.ADDRESS.Length>55)
								{
									ClientRow.ADDRESS=ClientRow.ADDRESS.Substring(0,55);
								}
							
								if(!DBNull.Value.Equals(p_PatientReader["address_line_2"])&&null!=p_PatientReader["address_line_2"])
								{
									ClientRow.ADDRESS+=(" "+p_PatientReader["address_line_2"].ToString());
									if(ClientRow.ADDRESS.Length>55)
									{
										ClientRow.ADDRESS=ClientRow.ADDRESS.Substring(0,55);
									}
							...
							if(DBNull.Value.Equals(p_PatientReader["city"])&&null!=p_PatientReader["city"]&&p_PatientReader["city"].ToString().Length<53)
							{
								ClientRow.CITY=p_PatientReader["city"].ToString();
							}
							if(DBNull.Value.Equals(p_PatientReader["state"])&&null!=p_PatientReader["state"]&&p_PatientReader["state"].ToString().Length==2)
							{
								ClientRow.STATE=p_PatientReader["state"].ToString();
							}
							if(DBNull.Value.Equals(p_PatientReader["zip"])&&null!=p_PatientReader["zip"])
							{
								ClientRow.ZIP=p_PatientReader["zip"].ToString().Replace("-","").Replace(" ","");
								if(ClientRow.ZIP.Length>4&&ClientRow.ZIP.Length!=5&&ClientRow.ZIP.Length!=9)
								{
									ClientRow.ZIP=ClientRow.ZIP.Substring(0,5);
								}
								else
								{
									ClientRow.ZIP="";
							...
							else if(!DBNull.Value.Equals(sdr_VaccineEligibility[0])&&null!=sdr_VaccineEligibility[0])
							{
								if(sdr_VaccineEligibility[0].ToString().ToUpper()=="V01"||
									...
									sdr_VaccineEligibility[0].ToString().ToUpper()=="V00")
								{
							...
							while(VaccineReader.Read())
							{
								try
								{
									AddGRITSVaccineData(VaccineReader["cpr_id"].ToString(), 
										VaccineReader["open_encounter_id"], 
										VaccineReader["treatment_id"], 
										VaccineReader["begin_date"], 
										VaccineReader["cpt_code"].ToString(), 
								...
								while(VaccineReader2.Read())
								{
									try
									{
										AddGRITSVaccineData(VaccineReader2["cpr_id"].ToString(), 
											VaccineReader2["open_encounter_id"], 
											VaccineReader2["treatment_id"], 
											VaccineReader2["begin_date"], 
											"     ", // cpt_code
											VaccineReader2["user_full_name"].ToString(), 
											VaccineReader2["description"].ToString(),
											VaccineReader2["common_name"].ToString());
										//MessageBox.Show(VaccineReader2["common_name"].ToString());
									}
									catch(Exception exc)
									{
										//MessageBox.Show("Error Getting Vaccine Data\r\n\r\n"+exc.ToString());
										util.LogEvent("EproLibNET.GRITS","PrintReport()","Error getting Vaccine Data\r\n\r\n"+exc.ToString(),4);
										continue;
									}
								}
							}
		...
		/// <exception cref="System.NullReferenceException">Thrown if CPR_ID param is null.</exception>
		private void AddGRITSVaccineData(string CPR_ID, object ENCOUNTER_ID, object TREATMENT_ID, object VACCINE_DATE, string CPT_CODE, string PROVIDER, string SITE, string COMMON_NAME)//<-changed 10/13/05
		{
