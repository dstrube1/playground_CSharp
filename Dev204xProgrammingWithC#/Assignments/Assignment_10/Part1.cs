...
//			samples.Linq4(); // This sample uses the where  clause to find all customers in Washington and then it 
		...
		public class LinqSamples
		{
				...
				try{
					var waCustomers =
						from cust in customers
							where cust != null && cust.City != null && cust.City == "Washington"
						select cust;
					...
					Console.WriteLine ("ArgumentNullException in Linq4. This occurs when trying to implement the sample code in Xamarin.");
				}
			}
			...
			public void Linq5()
			{
				...
				var longDigits = digits.Where((digit, index) => digit.Length > index);
				var equalDigits = digits.Where((digit, index) => digit.Length == index);
				...
				foreach (var d in longDigits)
				{
					Console.WriteLine("The word {0} is longer than its value.", d);
				}
				Console.WriteLine("Equal digits:");
				foreach (var d in equalDigits)
				{
					Console.WriteLine("The word {0} is equal to its value.", d);
				}
			}
